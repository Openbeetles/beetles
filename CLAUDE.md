...
- **编码前先做系统审查**：编码之前必须先审查现有代码；审查时必须完整审查调用链路并做系统全面的审查。实施编码时必须一步步推进，避免前后落差过大导致逻辑无法闭合、引入冗余或把问题扩散到新的链路。
- **旧代码妨碍主线时允许直接重写**：若既有实现已经妨碍系统主线、平台稳态、灵魂自治或可维护性，不要求强行兼容旧实现；可以直接删除、重写或做架构级翻修。
- **灵魂自治是最高原则**：禁止用 cue 词表、硬编码裁决、拍脑袋规则或其他外部逻辑去替代灵魂成长、关系治理与边界判断本体。
- **实现服从系统设计**：不是无脑敲功能、不是机械实现需求；每次实现都必须同时考虑系统设计、结构增强、代码优化、性能、平台差异、用户交互结果与代码质量。
- **每阶段都要有目的和收益**：开始一个阶段前必须先明确为什么做、做完系统会得到什么确定收益；不能无目标推进。
- **每结束一步必须回写文档**：不允许只改代码不回写文档；阶段或关键步骤结束后，必须同步回写主方案文档、阶段文档或相关设计文档，使文档与代码一致。
- **允许参考外部项目，但禁止照搬**：可以参考 `/Users/yiner/code/beetle/dev` 下的项目吸收机制与经验，但不能抄实现或叙事；目标是结合 Beetle 定位做出更强护城河。
- **必须考虑扩展性与维护性，但禁止过度设计**：抽象要服务真实问题，不能为了“看起来高级”而引入超过当前复杂度的设计。
...
- **禁止 cue 词表设计**：记忆保存、长期记忆召回、用户偏好识别等能力，禁止依赖固定触发词、硬编码关键词列表、短语白名单/黑名单或同类 cue 词表实现。应优先使用结构化记忆、程序性状态/阈值、摘要 grounding、检索归一化与必要的语义检索；不得用 cue 词表替代通用多语言语义判断。
