...
16MB S3 默认分区表 `partitions.csv` 现改为单 `factory` app 槽位 `0x20000/0x600000`，SPIFFS 保持在 `0x620000/0x9D0000`。除非明确执行迁移或格式化决策，否则不要再次改变 SPIFFS extent，因为 ESP-IDF 可能在文件系统 extent 改变后格式化已有用户配置。
