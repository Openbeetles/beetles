...
16MB S3 默认分区表 `partitions.csv` 保持 SPIFFS 起始地址在 `0xA20000`，大小为 `0x5D0000`；旧的独立唤醒资源区已移除。`ota_0` 为 `0x540000`，`ota_1` 为 `0x4C0000`；这是非对称 OTA 布局，新固件若超过 `ota_1` 大小，应通过串口/工厂刷写到 `ota_0` 或先缩小固件体积。
