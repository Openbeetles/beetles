# 中文文档

[返回文档总索引](../README.md)

这里是中文文档目录。

| 文档 | 适合什么时候看 |
|------|----------------|
| [configuration.md](configuration.md) | 第一次配网、打开配置页、设置配对码 |
| [config-api.md](config-api.md) | 自己写前端、脚本，或调用设备接口 |
| [tools.md](tools.md) | 想知道 Beetle Agent OS 现在能使用哪些工具 |
| [llm-providers.md](llm-providers.md) | 配置大模型服务商、`api_url` 和切换顺序 |
| [display.md](display.md) | 配 SPI 屏幕和显示仪表板 |
| [hardware.md](hardware.md) | 看支持板型、资源限制和排错 |
| [hardware-device-config.md](hardware-device-config.md) | 写 `hardware.json` 并启用 `device_control` |
| [architecture.md](architecture.md) | 了解模块结构和扩展点 |
| [linux-release-rollback.md](linux-release-rollback.md) | 看 Linux 版 Agent OS 的安装、打包和回滚说明 |

建议阅读顺序：

1. `configuration`
2. `config-api`
3. `tools`、`llm-providers`
4. `hardware`、`display`、`architecture`
