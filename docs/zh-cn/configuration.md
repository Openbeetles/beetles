...
本页说明 Beetle OS 的配置范围、访问方式与推荐配置顺序。
...
使用在线配置页、桌面壳或本地构建出的 Configure UI，然后把 **「设备地址」** 填为：

- 第一次使用时，连接默认热点 **Beetle**，通常使用 `http://192.168.4.1`
- 如果这是 Linux 嵌入式设备，且其 STA 侧已经落在 `192.168.4.0/24`，热点会避让到 `http://172.16.42.1`
- 如果设备已经在局域网里，使用它当前的局域网地址

## 配对码

配对码保护的是保存配置、重启、恢复设置和其他高影响写操作。
