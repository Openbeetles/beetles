...
2. `build.sh --package-profile`
   - user-facing and release-facing package entrypoint
   - selects a package by root feature set
3. `scripts/expand_cargo_features.py`
   - feature-closure inspection tool
   - expands a root feature set into the local Cargo feature closure
...
- the default official ESP package is `voice+vision+sensor`
- the default official Linux package is `linux-full`
...
| `websocket` | reserved websocket channel surface | off by default |
...
`build.sh` does not hand-maintain a fully expanded feature string. The current contract is:

- select a root feature set
- expand the local feature closure from `Cargo.toml`
...
| `voice+vision+sensor` | `default_runtime + capability_voice + capability_vision + capability_sensor` | default official ESP package |
| `linux-full` | `default + capability_office + dingtalk` | default official Linux package |
...
- the current `linux-full` contract is `default + capability_office + dingtalk`
- `capability_office` is default-off; that does not prohibit explicit re-enablement in future ESP packages
- if the expanded full feature set is needed, inspect it through `scripts/expand_cargo_features.py`; do not maintain a hand-written expanded list in another script or document
