...
- **Canonical list**: Whatever the device returns, generated by [`handlers/root.rs`](../../src/platform/http_server/handlers/root.rs). The list grows over time but may still **omit** routes implemented in [`dispatch.rs`](../../src/platform/http_server/router/dispatch.rs)—do not use `GET /` `endpoints` as the only source of truth. As of the current tree, these are commonly **not** listed in `endpoints` (verify on device):
