...
impl DocumentsProvider for WebDavProvider {
    ...
    fn search_documents(
        ...
        search_tree(
            ...
            &query,
            SearchTreePlan::new(
                vec![normalize_relative_path(&query.path)],
                SearchLimits::new(MAX_SEARCH_SCAN_ENTRIES, max_read_bytes),
            ),
            |http, dir| propfind_entries(http, credential, dir, 1),
            |_, entry| entry.clone(),
            |_, entry, _| entry.path.clone(),
            |http, entry, _resolved_entry, read_limit| {
                let raw = http_get_bytes(http, credential, &entry.path, read_limit)?;
