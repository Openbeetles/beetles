//! SPIFFS / state-root backed office runtime status store.
...
pub struct SpiffsOfficeRuntimeStatusStore {
    cache: Mutex<Option<BTreeMap<String, OfficeAccountRuntimeStatus>>>,
}

impl SpiffsOfficeRuntimeStatusStore {
    pub fn new() -> Self {
        Self {
...
impl Default for SpiffsOfficeRuntimeStatusStore {
    fn default() -> Self {
        Self::new()
    }
}

impl OfficeRuntimeStatusStore for SpiffsOfficeRuntimeStatusStore {
...
mod tests {
    ...
    fn runtime_status_roundtrips() {
        let store = SpiffsOfficeRuntimeStatusStore::new();
