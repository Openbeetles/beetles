...
mod tests {
    ...
    fn live_probe_guard_activates_for_unsettled_wifi_or_fragmentation() {
        ...
        assert!(
            !should_use_stale_snapshot(true, TlsFragmentationRisk::Healthy),
            "explicit channel connectivity refresh should not be downgraded only because configure-ui is open"
        );
        assert!(!should_use_stale_snapshot(
            true,
            TlsFragmentationRisk::Healthy
