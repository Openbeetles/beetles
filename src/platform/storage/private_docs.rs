//! storage 实现的私有文档工作区存储。单文件 memory/private_doc_workspaces.json。
...
pub struct StoragePrivateDocStore {
    store: ChatScopedCachedJsonMapStore<StoredPrivateDocWorkspace>,
}

impl StoragePrivateDocStore {
    pub fn new() -> Self {
        Self {
...
impl Default for StoragePrivateDocStore {
    fn default() -> Self {
        Self::new()
    }
}

impl PrivateDocStore for StoragePrivateDocStore {
