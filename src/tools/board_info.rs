...
impl Tool for BoardInfoTool {
    ...
    fn description(&self) -> &'static str {
        "Return a whole-device or whole-host status snapshot as JSON. ESP: chip, heap, IDF, SPIFFS. Linux: platform \"linux\" plus cpu_model, cpu_cores, mem_*, distro_pretty/distro_id, kernel_release, hostname, arch, storage, storage_media, os (/proc/version), uptime, pressure, and WiFi STA state. Use this for overall system status, resource pressure, distro, CPU/RAM, and storage topology. For one specific process or detailed Linux network diagnostics, prefer the dedicated process or network tools."
    }
    fn schema(&self) -> &str {
