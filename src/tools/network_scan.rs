...
impl Tool for NetworkScanTool {
    ...
    fn description(&self) -> &'static str {
        "WiFi-specific diagnostics. Op: wifi_scan (scan nearby APs), wifi_status (current WiFi connection info), connectivity_check (HTTP reachability test with latency). Prefer this for AP discovery or WiFi station checks. For broader end-to-end path diagnosis, use `diagnose` with `op=network`."
    }
