...
pub fn build() -> Result<EspIdfBuildOutput> {
    ...
    let bindgen = bindgen::Factory::from_cmake(&target.compile_groups[0])?;
    let bindgen = if let Some(sysroot) = &gcc_sysroot {
        bindgen.with_sysroot(sysroot)
    } else {
        bindgen.with_linker(&compiler)
    };
